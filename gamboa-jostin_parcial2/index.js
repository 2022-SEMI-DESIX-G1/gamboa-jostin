((Utils) => {
    const App = {
      htmlElements: {
        ...
        pokemonClear: document.querySelector("#limpiar"),
      },
      init: () => {
        ...
        App.htmlElements.pokemonClear.addEventListener(
          "click",
          App.handlers.pokemonClickClear
        );
      },
      handlers: {
        pokemonFinderFormOnSubmit: async (e) => {
          ...
          try {
            ...
            App.htmlElements.pokemonClear.style.visibility = "visible";
          } catch (error) {
        ...
        pokemonClickClear: (e) => {
          ...
          App.htmlElements.pokemonClear.style.visibility = "hidden";
        },
      },
      evolution: {
      ...
      templates: {
        ...
        pokemonCard: ({ id, name, weight, height, sprites, abilities }) => {
          ...
          const habilidad = abilities.map(
            ({ ability, is_hidden }) =>
              ` <li>${ability.name}${is_hidden ? eye : ""}</li>`
          );
          const evolution = App.htmlElements.evolutions;

          var evolutionList = evolution.map(
            (element) =>
              `<li>${element.species.name} ${element.is_baby ? baby : ""}</li> `
          );
          return `
          <div class="container3">
          <h1 class="tituloPokemon">${name} (${id})</h1> 
          <h6>sprites</h6> 
          <div class="sprites">  
               <img  class="imsprites"src="${
                 sprites.front_default}"><img class="imgsolapada imsprites" src="${sprites.back_default}">
          </div>
          <div class="evolution" >
            <p>evolution chain</p> 
            <ul>
      ...
      <h3>Habilidades</h3> 
      <ul class="habilidades"> ${habilidad.join("")} </ul>
      </div>
                  
                
          <!-- <h1>${name} (${id})</h1><ul><li>Weight: ${weight}</li><li>Height: ${height}</li></ul><img src="${
        ...
        <path d="M12.975 2.475C12.675 2.175 12.15 2.175 11.775 2.475L9.975 4.275C9.1828 3.98092 8.34501 3.8286 7.5 3.825C4.65 3.9 2.1 5.4 0.75 7.875C0.9 8.175 1.125 8.475 1.35 8.775C1.95 9.6 2.7 10.275 3.525 10.8L2.25 12.075C1.95 12.375 1.875 12.9 2.25 13.275C2.55 13.575 3.075 13.65 3.45 13.275L12.975 3.675C13.275 3.3 13.275 2.775 12.975 2.475ZM5.025 9.225L4.05 10.2C3.15 9.675 2.325 8.925 1.725 8.025C2.625 6.75 3.825 5.85 5.25 5.4C4.275 6.45 4.2 8.1 5.025 9.225ZM7.575 6.75C7.2 6.375 7.275 5.775 7.65 5.4C8.025 5.1 8.55 5.1 8.925 5.4L7.575 6.75ZM13.725 7.125C13.35 6.6 12.9 6.075 12.375 5.7L11.625 6.45C12.225 6.9 12.75 7.425 13.2 8.1C11.925 10.05 9.75 11.25 7.425 11.25H6.825L6.075 12C6.6 11.925 7.05 12 7.5 12C9.975 12 12.3 10.8 13.725 8.775C13.95 8.475 14.1 8.175 14.325 7.875C14.1 7.65 13.95 7.35 13.725 7.125ZM10.5 7.5L7.5 10.5C9.15 10.5 10.5 9.15 10.5 7.5Z" fill="black"/>
          ...
          const pokemonList = pokemon.map(
            ({ pokemon, is_hidden }) =>
              `<li>${pokemon.name} ${is_hidden ? eye : ""}</li>`
          );

          return `
          <div class="habilidades">
          <h1 class="tituloPokemonn">${name} (${id})</h1> 
          <p class="subtittle">¿Quién puede aprenderlo?</p><ul>${pokemonList.join(
            ""
          )}</ul></div>`;
        },
      },
